...
public abstract class HarvestHandlerAbstractCrop implements IHarvestHandler {
    ...
    public boolean harvestAndReplant(World world, IBlockAccess chunkCache, BlockPos pos, IBlockState state, IDrone drone) {
        ...
        List<EntityItem> seedItems = world.getEntitiesWithinAABB(EntityItem.class, new AxisAlignedBB(pos), entityItem -> this.isSeed(world, pos, state, entityItem.getItem()));
        if (!seedItems.isEmpty()) {
            seedItems.get(0).getItem().shrink(1);//Use a seed
            world.setBlockState(pos, this.withMinAge(state)); //And plant it.
            return true;
        } else {
            return false;
        }
