...
public class AreaTypeCylinder extends AreaType {
                        ...
                        if (centerDistSq <= radSq) {
                            for (int x = minX; x <= maxX; x++) {
                                if (centerDistSq >= innerRadiusSq ||
                                        ...
                                        this.cylinderType == EnumCylinderType.HOLLOW && (x == minX || x == maxX)) {
                    ...
                    for (int z = minZ; z <= maxZ; z++) {
                        ...
                        if (centerDistSq <= radSq) {
                            for (int y = Math.max(0, minY); y <= maxY && y < 256; y++) {
                                if (centerDistSq >= innerRadiusSq ||
                                        this.cylinderType == EnumCylinderType.FILLED ||
                                        this.cylinderType == EnumCylinderType.HOLLOW && (y == minY || y == maxY)) {
                    ...
                    for (int y = Math.max(0, minY); y <= maxY && y < 256; y++) {
                        ...
                        if (centerDistSq <= radSq) {
                            for (int z = minZ; z <= maxZ; z++) {
                                if (centerDistSq >= innerRadiusSq ||
                                        this.cylinderType == EnumCylinderType.FILLED ||
                                        this.cylinderType == EnumCylinderType.HOLLOW && (z == minZ || z == maxZ)) {
    ...
    public void addUIWidgets(List<AreaTypeWidget> widgets) {
        ...
        widgets.add(new AreaTypeWidgetEnum<>("gui.progWidget.area.type.cylinder.cylinderType", EnumCylinderType.class, () -> this.cylinderType, cylinderType -> this.cylinderType = cylinderType));
    ...
    public void writeToNBT(NBTTagCompound tag) {
        ...
        tag.setByte("cylinderType", (byte) this.cylinderType.ordinal());
    }
