...
public class ProgWidgetForEachCoordinate extends ProgWidgetAreaItemBase implements IJumpBackWidget, IJump,
    ...
    public IProgWidget getOutputWidget(IDroneBase drone, List<IProgWidget> allWidgets) {
        ...
        if (locations.size() > 0 && this.ai != null
                && (this.traversedPositions.size() == 1 || !this.aiManager.getCoordinate(this.elementVariable).equals(BlockPos.ORIGIN))) {
            BlockPos pos = this.ai.getCurCoord();
            if (pos != null) {
    ...
    public EntityAIBase getWidgetAI(IDroneBase drone, IProgWidget widget) {
        return this.ai = new DroneAIForEachCoordinate(drone, (ProgWidgetForEachCoordinate) widget);
    }
