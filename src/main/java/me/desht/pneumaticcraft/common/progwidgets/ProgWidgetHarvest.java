...
public class ProgWidgetHarvest extends ProgWidgetDigAndPlace implements IToolUser {
    ...
    public EntityAIBase getWidgetAI(IDroneBase drone, IProgWidget widget) {
        return this.setupMaxActions(new DroneAIHarvest(drone, (ProgWidgetAreaItemBase) widget), (IMaxActions) widget);
    }
    ...
    public void getTooltip(List<String> curTooltip) {
        ...
        if (this.requiresTool()) curTooltip.add(I18n.format("gui.progWidget.harvest.requiresHoe"));
    }

    @Override
    public void writeToNBT(NBTTagCompound tag) {
        super.writeToNBT(tag);
        tag.setBoolean("requireHoe", this.requireHoe);
    }

    @Override
    public void readFromNBT(NBTTagCompound tag) {
        super.readFromNBT(tag);
        this.requireHoe = tag.getBoolean("requireHoe");
    }
}
