...
public class AreaTypeRandom extends AreaType {
    ...
    public void addArea(Consumer<BlockPos> areaAdder, BlockPos p1, BlockPos p2, int minX, int minY, int minZ, int maxX, int maxY, int maxZ) {
        ...
        if (this.pickedAmount >= size) { //If we pick >= than there are blocks, just pick all blocks
    ...
    public void addUIWidgets(List<AreaTypeWidget> widgets) {
        ...
        widgets.add(new AreaTypeWidgetInteger("gui.progWidget.area.type.random.blocksSelected", () -> this.pickedAmount, amount -> this.pickedAmount = amount));
    }

    @Override
    public void writeToNBT(NBTTagCompound tag) {
        super.writeToNBT(tag);
        tag.setInteger("pickedAmount", this.pickedAmount);
    }

    @Override
    public void readFromNBT(NBTTagCompound tag) {
        super.readFromNBT(tag);
        this.pickedAmount = tag.getInteger("pickedAmount");
    }
