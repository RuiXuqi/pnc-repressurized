...
public class ContainerLogistics extends ContainerPneumaticBase {
    ...
    public ContainerLogistics(InventoryPlayer inventoryPlayer, SemiBlockLogistics logistics) {
        super(null);
        this.itemContainer = logistics == null;
        if (this.itemContainer) {
            logistics = getLogistics(inventoryPlayer.player, this.getHeldLogisticsFrame(inventoryPlayer.player));
        }
        ...
        if (logistics != null) {
            ...
            for (int y = 0; y < 3; y++) {
                for (int x = 0; x < 9; x++) {
                    this.addSlotToContainer(logistics.canFilterStack() ?
    ...
    public void onContainerClosed(EntityPlayer player) {
        if (this.itemContainer && this.logistics != null) {
            ItemStack logisticsStack = this.getHeldLogisticsFrame(player);
            if (!logisticsStack.isEmpty()) {
                NonNullList<ItemStack> drops = NonNullList.create();
                this.logistics.addDrops(drops);
    ...
    public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int slotIndex) {
        ...
        if (slotIndex >= this.playerSlotsStart && srcSlot != null && srcSlot.getHasStack()) {
