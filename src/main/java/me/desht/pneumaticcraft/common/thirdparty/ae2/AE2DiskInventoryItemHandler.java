...
public class AE2DiskInventoryItemHandler implements IInventoryItem {
    ...
    public void getStacksInItem(ItemStack stack, List<ItemStack> curStacks) {
        IMEInventoryHandler<IAEItemStack> cellInventoryHandler = this.cellRegistry.getCellInventory(stack, null, this.itemChannel);
        if (cellInventoryHandler != null) {
            IItemList<IAEItemStack> cellItemList = this.itemChannel.createList();
