...
public class GuiChargingStation extends GuiPneumaticContainerBase<TileEntityChargingStation> {
    ...
    public void initGui() {
        ...
        this.guiSelectButton = new GuiButtonSpecial(1, xStart + 89, yStart + 15, 21, 20, "");
    ...
    public void updateScreen() {
        ...
        this.guiSelectButton.visible = stack.getItem() instanceof IChargingStationGUIHolderItem;
        if (this.guiSelectButton.visible) {
            this.guiSelectButton.setTooltipText("Manage upgrades for the " + stack.getDisplayName());
        }

        // multiplier of 25 is about the max that looks good (higher values can make the animation look like
        // it's going the wrong way)
        if (this.te.charging) {
            this.renderAirProgress += 0.001F * Math.min(25f, this.te.getSpeedMultiplierFromUpgrades());
            if (this.renderAirProgress > 1f) this.renderAirProgress = 0f;
        } else if (this.te.discharging) {
            this.renderAirProgress -= 0.001F * Math.min(25f, this.te.getSpeedMultiplierFromUpgrades());
            if (this.renderAirProgress < 0f) this.renderAirProgress = 1f;
        }
    }
    ...
    protected Point getGaugeLocation() {
        int xStart = (this.width - this.xSize) / 2;
        int yStart = (this.height - this.ySize) / 2;
        return new Point(xStart + this.xSize * 3 / 4 + 10, yStart + this.ySize / 4 + 4);
    }
    ...
    protected void addPressureStatInfo(List<String> pressureStatText) {
        ...
        if (this.te.charging || this.te.discharging) {
            pressureStatText.add("\u00a77" + (this.te.charging ? "C" : "Disc") + "harging at");
            pressureStatText.add("\u00a70" + (double) Math.round(PneumaticValues.CHARGING_STATION_CHARGE_RATE * this.te.getSpeedMultiplierFromUpgrades()) + "mL/tick");
        }
    }
    ...
    protected void addProblems(List<String> textList) {
        ...
        ItemStack chargeStack = this.te.getPrimaryInventory().getStackInSlot(TileEntityChargingStation.CHARGE_INVENTORY_INDEX);
    ...
    protected void addWarnings(List<String> curInfo) {
        ...
        if (chargeStack.isEmpty()) {
            ...
            if (chargeItem.getPressure(chargeStack) > this.te.getPressure() + 0.01F && chargeItem.getPressure(chargeStack) <= 0) {
            ...
            } else if (chargeItem.getPressure(chargeStack) < this.te.getPressure() - 0.01F && chargeItem.getPressure(chargeStack) >= chargeItem.maxPressure(chargeStack)) {
    ...
    private void renderAir() {
        ...
        for (int i = 0; i < particles; i++) {
            this.renderAirParticle(this.renderAirProgress % (1F / particles) + (float) i / particles);
        }
    ...
    private void renderAirParticle(float particleProgress) {
        ...
        wr.pos(x, y, this.zLevel).endVertex();
