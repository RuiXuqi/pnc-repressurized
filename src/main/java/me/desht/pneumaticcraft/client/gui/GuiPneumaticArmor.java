...
public class GuiPneumaticArmor extends GuiPneumaticInventoryItem {
    ...
    public GuiPneumaticArmor(ContainerChargingStationItemInventory container, TileEntityChargingStation te) {
        super(container, te);
        this.registryName = this.itemStack.getItem().getRegistryName().getPath();
        this.equipmentSlot = ((ItemArmor) this.itemStack.getItem()).armorType;
    }

    @Override
    public void initGui() {
        super.initGui();
        this.addAnimatedStat("gui.tab.info", Textures.GUI_INFO_LOCATION, 0xFF8888FF, true).setText("gui.tab.info.item." + this.registryName);
        this.statusStat = this.addAnimatedStat("Status", this.itemStack, 0xFFFFAA00, false);
    ...
    private List<String> getStatusText() {
        ...
        text.add("\u00a77Air Usage:");
        float totalUsage = UpgradeRenderHandlerList.instance().getAirUsage(FMLClientHandler.instance().getClient().player, this.equipmentSlot, true);
        if (totalUsage > 0F) {
            ...
            for (int i = 0; i < renderHandlers.size(); i++) {
                if (CommonArmorHandler.getHandlerForPlayer(player).isUpgradeRendererInserted(this.equipmentSlot, i)) {
                    IUpgradeRenderHandler handler = renderHandlers.get(i);
                    float upgradeUsage = handler.getEnergyUsage(CommonArmorHandler.getHandlerForPlayer(player).getUpgradeCount(this.equipmentSlot, EnumUpgrade.RANGE), player);
                    if (upgradeUsage > 0F) {
                        text.add(TextFormatting.BLACK + PneumaticCraftUtils.roundNumberTo(upgradeUsage, 1) + " mL/tick (" + handler.getUpgradeName() + ")");
                    }
                }
            }
            text.add("\u00a70--------+");
        ...
        int volume = UpgradableItemUtils.getUpgrades(EnumUpgrade.VOLUME, this.itemStack) * PneumaticValues.VOLUME_VOLUME_UPGRADE + this.getDefaultVolume();
        int airLeft = (int) (((IPressurizable) this.itemStack.getItem()).getPressure(this.itemStack) * volume);
        if (totalUsage == 0) {
    ...
    protected int getDefaultVolume() {
        return ((ItemPneumaticArmor) this.itemStack.getItem()).getBaseVolume();
    }
}
