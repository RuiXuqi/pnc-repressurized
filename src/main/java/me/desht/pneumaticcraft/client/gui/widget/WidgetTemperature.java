...
import static me.desht.pneumaticcraft.api.recipe.TemperatureRange.TemperatureScale.CELSIUS;
...
public class WidgetTemperature extends WidgetBase {
    ...
    public WidgetTemperature(int id, int xIn, int yIn, TemperatureRange totalRange, int initialTemp, int tickInterval) {
        super(id, xIn, yIn, 13, 50);
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        drawTicks();
        if (drawText) {
            GuiUtils.drawScaledText(Minecraft.getMinecraft().fontRenderer, CELSIUS.symbol(), x + 7, y + height + 1, 0xFF404040, 0.5f);
        }
    ...
    private void drawTicks() {
        ...
        while (tickTempC <= totalRange.getMax() - 273) {
            ...
            GuiUtils.hLine(x + 4, x + 6, y - 1 + height - yOffset, 0xA0404040);
            if (yOffset != 0 && yOffset != height - 1) {
                GuiUtils.hLine(x + 6, x + 8, y - 1 + height - yOffset, 0x80C0C0C0);
            }
            if (drawText && n % 2 == 0) { // && (tickInterval > 10 || n > 0)) {
                String s = String.valueOf(tickTempC);
    ...
    private void drawOperatingTempMarkers() {
        if (operatingRange != null) {
            if (totalRange.inRange(operatingRange.getMax())) {
                int yOffset = getYPos(operatingRange.getMax());
                GuiUtils.hLine(x + 7, x + 11, y + 1 + height - yOffset, 0xFFE0E040);
                GuiUtils.hLine(x + 9, x + 9,  y     + height - yOffset, 0x80E0E040);
                GuiUtils.hLine(x + 8, x + 10, y - 1 + height - yOffset, 0x80E0E040);
                GuiUtils.hLine(x + 7, x + 11, y - 2 + height - yOffset, 0x80E0E040);
            }
    ...
    public void addTooltip(int mouseX, int mouseY, List<String> curTip, boolean shift) {
        curTip.add(I18n.format("gui.tooltip.temperature", temperature - 273));
        if (operatingRange != null && showOperatingRange) {
            ...
            curTip.add(tf + I18n.format("gui.tooltip.temperatureRequired", operatingRange.asString(CELSIUS)));
        }
    }
    ...
    public static WidgetTemperature fromOperatingRange(int id, int x, int y, TemperatureRange range) {
        ...
        return new WidgetTemperature(id, x, y, totalRange, range.getMin(), interval).setOperatingRange(range);
    }
