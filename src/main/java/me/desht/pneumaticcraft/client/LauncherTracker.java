...
public enum LauncherTracker {
    ...
    public boolean isPlayerOKToLaunch() {
        if (this.launcherProgress > 0) return false;
    ...
    public void chargeLauncher() {
        if (this.launcherProgress > 0 && this.launcherProgress < MAX_PROGRESS) {
            this.launcherProgress++;
        }
    }

    public void trigger() {
        NetworkHandler.sendToServer(new PacketChestplateLauncher((float) this.launcherProgress / (float) MAX_PROGRESS));
    ...
    public void render(ScaledResolution sr, float partialTicks) {
        ...
        float progress = Math.min(100f, (this.launcherProgress + partialTicks) * 100f / LauncherTracker.MAX_PROGRESS);
