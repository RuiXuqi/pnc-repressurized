...
public abstract class AbstractModelRenderer<T extends TileEntityBase> extends TileEntitySpecialRenderer<T> {
    ...
    public void render(T te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
        ...
        if (!this.shouldRender(te) || te.getWorld().getChunkProvider().provideChunk(te.getPos().getX() >> 4, te.getPos().getZ() >> 4).isEmpty())
            return;

        GlStateManager.pushMatrix();
        FMLClientHandler.instance().getClient().getTextureManager().bindTexture(this.getTexture(te));
        ...
        this.renderModel(te, partialTicks);

        GlStateManager.popMatrix();

        this.renderExtras(te, x, y, z, partialTicks);
    }
