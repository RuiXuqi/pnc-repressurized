...
public class RenderRing extends RenderProgressingLine {
    ...
    public void renderInterpolated(RenderProgressingLine lastTickLine, float partialTick, float rotationYaw, float rotationPitch) {
        ...
        double renderProgress = this.getInter(this.progress, lastTickLine.progress, partialTick);
        GlStateManager.translate((this.getInter(this.endX, lastTickLine.endX, partialTick) - this.startX) * renderProgress, (this.getInter(this.endY, lastTickLine.endY, partialTick) - this.startY) * renderProgress, (this.getInter(this.endZ, lastTickLine.endZ, partialTick) - this.startZ) * renderProgress);
        ...
        RenderUtils.glColorHex(0xFF000000 | this.color);
        double size = 5 / 16D;
